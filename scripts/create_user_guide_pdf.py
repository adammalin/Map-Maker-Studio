...
def build_pdf(output):
    ...
    pdf.setSubject("macOS and Windows setup, multi-row label callouts, canvas navigation, map layers, CSV import, custom SVG pins, ORNL color swatches, exports, and local MCP control")
    ...
    wrapped(pdf, "Hold Space and drag anywhere to pan; drag a pin without Space to move it. Drag a callout to place all its label rows and create an automatic leader line; dragging locks it. Scroll at the pointer or use -/+ to zoom; 0 Fits and 1 returns to 100%. Project keys: Cmd/Ctrl+S Save, Cmd/Ctrl+O Open, Cmd/Ctrl+N New, Cmd/Ctrl+Z Undo, / Search, and ? Shortcuts.", 56, 119, 500, font="Helvetica-Bold", size=7.6, leading=10.1, color=WHITE)
    footer(pdf, 2)
    pdf.showPage()
    ...
    bullet(pdf, "Open project", "Schema 5 stores every label row, font, size, position, lock, and leader line. Versions 1-4 migrate safely when opened.", 318, y2 - 58, 254, FORGE)
    pdf.setFillColor(SOFT_GREEN)
    ...
    step_card(pdf, "4", "JSON", "The complete project: label formatting and placement, layers, data, and embedded pin artwork.", 442, 516, 130, 92, SOFT_GREEN, GREEN)
