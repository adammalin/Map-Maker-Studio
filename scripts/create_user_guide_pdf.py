...
def build_pdf(output):
    ...
    pdf.setSubject("macOS and Windows setup, label views, bulk editing, recovery history, export preflight, map layers, CSV import, custom SVG pins, ORNL color swatches, and local MCP control")
    ...
    step_card(pdf, "1", "MAP EDITOR", "Choose Pins, City, City + Company, selected-layer, or selected-location labels; then arrange and review lines.", 40, 190, 120, 96)
    step_card(pdf, "2", "LOCATIONS", "Open Bulk edit for City, Company, layer, and visibility; use the inspector for exact row styling.", 174, 190, 120, 96, SOFT_NAVY, NAVY)
    ...
    wrapped(pdf, "Hold Space and drag anywhere to pan; drag a pin without Space to move it. Dragging a callout locks its saved position. Arrange labels checks text collisions and crossed leaders. Scroll at the pointer or use -/+ to zoom; 0 Fits and 1 returns to 100%. Project keys: Cmd/Ctrl+S Save, Cmd/Ctrl+O Open, Cmd/Ctrl+N New, Cmd/Ctrl+Z Undo, / Search, and ? Shortcuts.", 56, 119, 500, font="Helvetica-Bold", size=7.35, leading=9.65, color=WHITE)
    footer(pdf, 2)
    pdf.showPage()
    ...
    y2 = bullet(pdf, "Map unfamiliar columns", "Open Column mapping in the import review when a client uses headings such as Town or Vendor. Then choose a target layer and Add or Replace.", 318, 456, 254)
    y2 = bullet(pdf, "Bulk edit", "Use the data table to change City, State, Company, layer, pin visibility, or callout visibility across selected rows.", 318, y2 - 5, 254, BLUE)
    bullet(pdf, "Save and recover", "Schema 6 stores label views and complete map data. Version history retains up to 24 manual or automatic recovery points.", 318, y2 - 58, 254, FORGE)
    pdf.setFillColor(SOFT_GREEN)
    ...
    bullet(pdf, "2  Arrange and review labels", "Run Arrange labels, review text and line warnings, then drag and lock exceptions. Exports reuse the active saved label view.", 40, 402, 532, BLUE)
    bullet(pdf, "3  Run export preflight", "Review hidden data, missing Company names, canvas bounds, line crossings, fonts, and PowerPoint editability before writing output.", 40, 348, 532, FORGE)
    label(pdf, "Troubleshooting", 40, 292)
    wrapped(pdf, "Setup fails: rerun the setup script; it repairs dependencies and rebuilds safely. A city is unresolved: add exact coordinates or use the official Census place name. Missing company: use Bulk edit or switch to City names. Crossed leader: run Arrange labels, then drag and lock an exception. Autosave fails: keep the app open, check the status chip, and create a recovery point before continuing.", 40, 273, 532, size=8.35, leading=11.2)
    pdf.setFillColor(NAVY)
