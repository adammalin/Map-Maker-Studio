...
test("CSV import resolves city and state rows offline", () => {
  const result = parseLocationsCsv("City,State,Label,Pin Color\nOak Ridge,TN,Lab,#00662c\nSeattle,WA,Northwest,#006ba6\n");
...
test("CSV import preserves coordinates and custom columns", () => {
  const result = parseLocationsCsv("city,state,lat,lng,pin_type,show_label,owner\nDenver,CO,39.7392,-104.9903,diamond,no,Central team\n");
